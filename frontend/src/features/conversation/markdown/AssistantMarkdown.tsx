...
// Math with `$...$` enabled. Off by default (see `inlineMath` below); a chat
// message mentioning "$5 ... $10" would otherwise be parsed as an equation.
...
type AssistantMarkdownProps = {
   ...
   * only recognises `$$` unless told otherwise. Lesson bodies are math in every
   * subject (the writer model emits `$f$`, `$x^2$` inline as a matter of course)
   * and a single `$` there is not punctuation but a swallowed formula — the
   * backslashes get eaten, so `$\bar{v}$` reaches the learner as `$ar{v}$`.
   */
  inlineMath?: boolean
  /**
