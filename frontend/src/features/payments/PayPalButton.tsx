...
export function PayPalButton({
  ...
  useEffect(() => {
    ...
    loadPayPalSdk(PAYPAL_CLIENT_ID)
      .then(() => {
        ...
        return window.paypal
          .Buttons({
            ...
            onError: () => {
              handlers.current.onError(t('credits.paypal.incomplete'))
            },
          })
      ...
      .catch((err: unknown) => {
        setSdkError(
          err instanceof Error ? err.message : t('credits.paypal.sdkFailed')
        )
      })
  ...
  }, [ready, pack.id, pack.priceUsd, pack.credits, t])
  ...
  if (!ready) {
    return (
      ...
      >
        <PayPalMark />
        <span>{t('credits.paypal.notReady')}</span>
      </button>
    )
  }
