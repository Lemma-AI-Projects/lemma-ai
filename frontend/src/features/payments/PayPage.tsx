...
import { CREDIT_PACKS, creditPackPerks, creditPackTagline } from './plans'
import type { CreditPack } from './types'
import { useBalance, usePaymentConfig } from './usePayments'

// main-v3 没有 i18n 体系（本分支组件一律写死中文），所以这里直接把文案内联，
// 不再经过 useAppTranslation。若日后本分支引入 i18n，回到 main-v2 的
// features/payments/PayPage.tsx 取回 t() 版本即可。
const T = {
  eyebrow: 'Credits',
  title: '为你的学习力充值',
      ...
      <div className="mx-auto max-w-5xl px-6 py-10 md:px-10">
        ...
        <header className="pay-fade-up flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="mt-1.5 max-w-md text-sm text-zinc-500">{T.subtitle}</p>
          </div>
        ...
        {successCredits != null && (
          <div className="mt-6 flex items-center gap-3 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-800">
            ...
            {`充值成功，已到账 +${successCredits.toLocaleString()} credits`}
          </div>
        )}
        ...
        {/* 页脚说明 */}
        <p className="mt-10 text-center text-xs text-zinc-500">{T.footer}</p>
      </div>
    </div>
  )
}
  ...
  return (
    ...
    >
      ...
      <div className="flex items-baseline justify-between">
        ...
        <span className="text-xs text-zinc-400">{creditPackTagline(pack.id)}</span>
      </div>
      ...
      <ul className="mt-5 flex-1 space-y-2.5">
        {creditPackPerks(pack.id).map((perk) => (
          <li
            key={perk}
            className={cn(
      ...
      <div className="mt-6 flex flex-col gap-2">
        <StripeCheckoutButton pack={pack} ready={stripeReady} onError={onError} />
        <PayPalButton
          pack={pack}
          ready={paypalReady}
          onSuccess={onSuccess}
