...
# This branch's course status machine is ('intake', 'organizing',
# 'materializing', 'ready', 'failed') — the four-level rebuild narrowed it, and
# `building` was main-v2's spelling of the same moment. `materializing` is the
# honest translation: the row exists, it is not enterable, and generation is
# still running. The free-course client does not care about the literal (it
# treats anything that is not terminal as still-building), so mapping here is
# enough and this branch's CHECK constraint stays untouched.
_BUILDING = "materializing"
_READY = "ready"
_FAILED = "failed"
