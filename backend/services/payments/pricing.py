...
The frontend's `plans.ts` is display-only. Money and credit amounts are
recomputed here from the pack id, so a tampered client request can never change
what the user is charged or how many credits land. Keep in sync with the
business-finalized table; the frontend copy is cosmetic.
"""
...
# Initial (dev) pricing — placeholder until the business table is final.
