...
_DEFAULT_AI_ROUTES_JSON = (
    ...
    # Free Course: one route per pipeline step. The course is written by a chain
    # of small structured calls (intent -> map -> gap -> blueprint -> lesson),
    # each with its own prompt and output shape; feedback and the two teaching
    # session calls are learner-facing, hence the shorter timeouts on the turn.
    # Channels stay inside this branch's tested set (aihubmix -> openrouter):
    # main-v2 fell back to deepseek, which is not a channel this branch allows.
'], "free_course_intake": ['
    '{"platform": "aihubmix", "adapter": "openai_compatible",'
    ' "model": "gemini-2.5-flash", "priority": 0, "timeout_s": 60},'
    ' {"platform": "openrouter", "adapter": "openrouter",'
...
'], "free_course_lesson": ['
    ...
    ' {"platform": "openrouter", "adapter": "openrouter",'
    ' "model": "google/gemini-2.5-flash", "priority": 1, "timeout_s": 120}'
'], "free_course_feedback": ['
    '{"platform": "aihubmix", "adapter": "openai_compatible",'
    ' "model": "gemini-2.5-flash", "priority": 0, "timeout_s": 60},'
    ' {"platform": "openrouter", "adapter": "openrouter",'
    ' "model": "google/gemini-2.5-flash", "priority": 1, "timeout_s": 60}'
