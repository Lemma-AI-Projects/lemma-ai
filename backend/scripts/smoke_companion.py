...
DB 段（需 DB + 一门已建好的课程，--offline 跳过；无合适章节则自动 SKIP）：
  - point_gemini_files 状态机：claim(首次成功/再次被拒) -> mark_ready -> read_usable
    命中 -> 候选不匹配/过期返回 None；
...
def offline_checks() -> None:
    ...
    check(
        bool(template.strip()) and "load_point_video" in template,
        "companion 模板非空且说明可调 load_point_video 工具",
    )
    ...
    request = CompanionChatRequest.model_validate(
        {"pointId": str(uuid.uuid4()), "message": "为什么这里要这样推导？"}
    )
    check(
        isinstance(request.point_id, uuid.UUID) and request.conversation_id is None,
        "CompanionChatRequest camelCase 解析",
    )
    no_point = CompanionChatRequest.model_validate({"message": "纯文本提问"})
    check(
        no_point.point_id is None,
        "CompanionChatRequest pointId 可选(可为 null)",
    )
...
async def db_checks() -> None:
    ...
    async with AsyncSessionLocal() as db:
        row = (
            await db.execute(
                select(
                    CoursePoint.id,
                    CoursePoint.chosen_candidate_id,
    ...
    if row is None:
        ...
        return
    point_id, candidate_id, course_id = row
    print(f"  fixture point_id={point_id} candidate_id={candidate_id}")

    # --- point_gemini_files 状态机 ---
    try:
        async with AsyncSessionLocal() as db:
            await db.execute(
                delete(PointGeminiFile).where(
                    PointGeminiFile.point_id == point_id
                )
            )
            await db.commit()
        ...
        async with AsyncSessionLocal() as db:
            await gemini_file_service.mark_ready(
                db, point_id=point_id, candidate_id=candidate_id, video=fresh
            )
        async with AsyncSessionLocal() as db:
        ...
        async with AsyncSessionLocal() as db:
            mismatched = await gemini_file_service.read_usable(
                db, point_id=point_id, candidate_id=uuid.uuid4()
            )
    ...
    async with AsyncSessionLocal() as db:
        foreign_course = await companion_service.prepare_turn(
            db,
            CompanionChatRequest(point_id=point_id, message="hi"),
            user,
    ...
    async with AsyncSessionLocal() as db:
        bad_point = await companion_service.prepare_turn(
            db,
            CompanionChatRequest(point_id=uuid.uuid4(), message="hi"),
            user,
            course_id=course_id,
        )
    check(
        bad_point is not None and bad_point.candidate_id is None,
        "prepare_turn: 课程内无该章节 -> context(candidate=None, 不再 404)",
    )

    # pointId 省略(不在学习点上) 也合法 —— context.point_id/candidate 均 None。
    async with AsyncSessionLocal() as db:
        no_point_ctx = await companion_service.prepare_turn(
            db,
    ...
    check(
        no_point_ctx is not None
        and no_point_ctx.point_id is None
        and no_point_ctx.candidate_id is None,
        "prepare_turn: 无 pointId -> context(纯文本)",
    )
