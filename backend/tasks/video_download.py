"""Celery task: re-host a learning point's chosen video in Supabase Storage.

Celery 纪律: asyncio.run wraps the async body, args are JSON-safe (point_id as
str), the boto3 S3 client is built per task, and the module-level engine is
disposed at the end so the next task in the same worker process starts with a
clean connection pool.

Flow: resolve the point's chosen candidate -> atomically claim the asset row
...
def _storage_key(point_id: uuid.UUID) -> str:
    # One stable key per learning point: a re-pick re-downloads and overwrites
    # in place, so no orphaned objects accumulate. Always .mp4 (backends remux).
    return f"points/{point_id}.mp4"


async def run_download(point_id: uuid.UUID) -> None:
    ...
    try:
        ...
        if target is None:
            logger.info("point %s has no chosen video; skip", point_id)
            return

        async with AsyncSessionLocal() as db:
            claimed = await video_asset_service.claim_for_download(
                db, point_id=point_id, candidate_id=target.candidate_id
            )
        if not claimed:
            logger.info("point %s already downloading/ready; skip", point_id)
            return

        key = _storage_key(point_id)
        try:
            ...
            logger.warning(
                "point %s video download/upload failed: %s", point_id, exc
            )
            async with AsyncSessionLocal() as db:
...
@celery_app.task(
    name="video.download_point", bind=True, max_retries=2, default_retry_delay=30
)
def download_point_video(self, point_id: str) -> None:  # noqa: ANN001 — celery bind
    """Sync Celery entrypoint. Idempotent: a duplicate delivery or retry of an
    already-downloading/ready point is a no-op (the claim guards it)."""
    try:
        asyncio.run(run_download(uuid.UUID(point_id)))
