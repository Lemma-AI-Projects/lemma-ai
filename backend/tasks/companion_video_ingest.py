"""AI 伴学 Celery task: re-host a chapter's video INTO the Gemini Files API.

The companion needs the chapter's video as a Gemini file reference (file_uri) to
"see" it. The video body already lives in Supabase Storage (chapter_video_assets,
re-hosted by tasks/video_download.py); this task pulls that body down and uploads
it to the Files API, caching the result in chapter_gemini_files (~48h lifetime).
...
marks the cache row failed and re-raises so Celery retries; the companion ask
poll then reports preparing/failed accordingly.
"""
...
logger = logging.getLogger("lemma.tasks.companion_video_ingest")
...
async def run_ingest(chapter_id: uuid.UUID) -> None:
    ...
    try:
        ...
        if stored is None:
            ...
            # nothing to upload. Mark failed so the companion poll stops waiting;
            # a later ask (once the video is ready) re-enqueues a fresh attempt.
        ...
        try:
            with tempfile.TemporaryDirectory(prefix="lemma_companion_") as tmp_dir:
            ...
            logger.warning(
                "chapter %s gemini ingest failed: %s", chapter_id, exc
            )
            async with AsyncSessionLocal() as db:
                await gemini_file_service.mark_failed(
...
@celery_app.task(
    name="companion.ingest_video", bind=True, max_retries=2, default_retry_delay=30
)
