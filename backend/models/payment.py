...
class Payment(Base):
    ...
    `provider_order_id` is UNIQUE per provider (one row per order), the `status`
    state machine only moves forward, and the capture path takes a row lock
    before granting credits. Any replay — a retried frontend capture or a
    duplicated webhook — lands on an already-`captured` row and is skipped, so
    ...
    # "paypal" today; kept as a column so a second channel needs no migration.
...
class PaymentWebhookEvent(Base):
    ...
    `provider_event_id` is UNIQUE, so a replayed webhook is recorded exactly
    once and never granted twice.
    """
...
class CreditLedger(Base):
    ...
    latest row's value — no separate counter to drift out of sync.
    """
