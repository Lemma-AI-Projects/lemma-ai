...
class PointVideoAsset(Base):
    """The re-hosted, playable copy of a learning point's chosen video.

    One row per point (1:1). Kept separate from point_video_candidates (the
    ...
    sliding-expiry clock — so re-download, cleanup and reuse (feeding the file
    to AI video understanding) all hang off one record without polluting the
    funnel or the course tree.

    candidate_id pins WHICH chosen candidate was downloaded: when a point is
    re-picked (point.chosen_candidate_id changes) the asset turns stale and is
    ...
    __tablename__ = "point_video_assets"
    __table_args__ = (
        CheckConstraint(
        ...
        Index("ix_point_video_assets_last_accessed_at", "last_accessed_at"),
    )

    id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), primary_key=True, default=uuid.uuid4
    )
    # 1:1 with the point — UNIQUE so it never accumulates two assets.
